...
@property (nonatomic, assign, readwrite) BOOL stoppedUsingVad;
...
- (void)vadSpeechFrame:(NSData *)samples {
    ...
    for(int sample_offset=0; sample_offset+self->vad_state->samples_per_frame < size/2; sample_offset+=self->vad_state->samples_per_frame){
        ...
        if ( detected_speech == 1){
            ...
            NSLog(@"start talking...");
            NSLog(@"Starting......................");
            [self.delegate vadStartedTalking: samples];
...
- (instancetype)init {
    ...
    int vadSensitivity = (int)MIN(100,MAX(0,0)); //must be between 0 and 100
    int vadTimeout = (int)7000;
    
    self->vad_state = wv_detector_cvad_init(16000,0,7000);
